            ...
            <c:forEach var="course" items="${courses}" varStatus="count">
                <div class="list-group-item box">
                    <div class="col-md-8" style="margin-top: 3%">
                        ...
                        <p class="list-group-item-text" style="overflow: hidden;height: 6.6em
...
<script>
    $(document).ready(function () {
        $('.urlPath').val($(location).attr('pathname'));
        var urls = $(location).attr('pathname').split("/")
        var urlPath = "";
        for(i = 0 ; i < urls.length-1;i++) {
            urlPath = urlPath + "/" + urls[i+1];
        }
        $('.pagination').jqPagination({
            link_string: 'http://lazyeng.com/service/course/search/${keyword}/{page_number}',
            max_page: ${maxpage},
            current_page : ${currentPage},
            paged: function (page) {
                window.location = 'http://lazyeng.com/service/course/search/'+'${keyword}'+"/"+page;
            }
        });
    });
</script>
